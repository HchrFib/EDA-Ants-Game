...
*/
#define PLAYER_NAME Hormigator14
...
struct PLAYER_NAME : public Player {
    ...
    */
    ...
    PILA find_a_place_to_lay_eggs(Pos p , int id_queen, const map<int, My_player_Info>& citizens ) { 
        ...
        while(not Q.empty()) {
            ...
            if(cell(p1).id == -1) {
                ...
                for(Dir d : dirs) { 
                    if(pos_ok(p1 + d) and citizens.find(cell(p1 + d).id) == citizens.end() and dist[(p1+d).i][(p1+d).j] == UNDEF and cell(p1 + d).type != Water) {
            ...
            if(move_ok(p)) {
                ...
                if(h_id.second == Worker) {
                    ...
                    if(queen.reserve[0] >= 1 and queen.reserve[1] >= 1 and queen.reserve[2] >= 1) {
                        Ruta = find_a_place_to_lay_eggs(p, h_id.first, citizens);
                        if(not Ruta.empty()) {
                            coordenada = Ruta.top();
                            Ruta.pop();
                            ...
                            if(not M[aux.i][aux.j]) {
                                if(soldiers_id.size() < 3 and queen.reserve[0] >= 3 and queen.reserve[1] >= 3 and queen.reserve[2] >= 3) lay(h_id.first, coordenada.moviment, Soldier);
                                else lay(h_id.first, coordenada.moviment, Worker);
                                M[aux.i][aux.j] = 1;
                            } 
                        }
                    } else {
